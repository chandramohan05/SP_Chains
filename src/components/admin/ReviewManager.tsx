...
export function ReviewManager() {
  const [reviews, setReviews] = useState<(Review & { dealer_name?: string; product_name?: string })[]>([]);
  ...
  if (loading) return <p className="text-center mt-12 text-slate-500">Loading reviews...</p>;
        ...
        <div className="space-y-3">
          {filteredReviews.map((review) => (
            ...
            >
              ...
              {review.review_text && (
                <p className="text-sm text-slate-700 leading-relaxed mt-2">{review.review_text}</p>
              )}
            </div>
          ))}
        </div>
