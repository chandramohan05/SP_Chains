...
export function PricingManager() {
  ...
  const [mcxRate, setMcxRate] = useState(mockPricing.mcx_rate.toString());
  const [premiumPercentage, setPremiumPercentage] = useState(mockPricing.premium_percentage.toString());
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-1">
              Current: ₹{pricing.mcx_rate.toFixed(2)}/g
            </p>
          </div>
          ...
          <div>
            ...
            />
            <p className="text-xs text-slate-500 mt-1">
              Current: {pricing.premium_percentage.toFixed(2)}%
            </p>
          </div>
        </div>

        <div className="p-4 bg-slate-50 rounded-lg mb-6">
          <h4 className="font-medium text-slate-900 mb-3">Pricing Formula</h4>
          <div className="space-y-2 text-sm text-slate-600">
            <p>
              <span className="font-medium">Net Rate (Wholesale):</span> MCX Rate × (1 + Premium %)
            </p>
            <p>
              <span className="font-medium">Retail Rate:</span> Net Rate × 1.01 (1% markup)
            </p>
            <p className="pt-2 border-t border-slate-200">
              <span className="font-medium">Example with current values:</span><br />
              Net Rate = ₹{mcxRate || '0'} × (1 + {premiumPercentage || '0'}%) = ₹{calculateNetRate().toFixed(2)}/g<br />
